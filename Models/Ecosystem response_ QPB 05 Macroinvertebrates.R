...
log_likelihood_mod.1 <- sum(dnorm(data$macros_QPB, mean = fitted(mod.1), sd = sqrt(sum((data$macros_QPB - fitted(mod.1))^2) / (length(data$macros_QPB) - 2)), log = TRUE))
log_likelihood_mod.2 <- sum(dnorm(data$macros_QPB, mean = fitted(mod.2), sd = sqrt(sum((data$macros_QPB - fitted(mod.2))^2) / (length(data$macros_QPB) - 2)), log = TRUE))
log_likelihood_mod.3 <- sum(dnorm(data$macros_QPB, mean = fitted(mod.3), sd = sqrt(sum((data$macros_QPB - fitted(mod.3))^2) / (length(data$macros_QPB) - 2)), log = TRUE))
log_likelihood_mod.4 <- sum(dnorm(data$macros_QPB, mean = fitted(mod.4), sd = sqrt(sum((data$macros_QPB - fitted(mod.4))^2) / (length(data$macros_QPB) - 2)), log = TRUE))
log_likelihood_mod.5 <- sum(dnorm(data$macros_QPB, mean = fitted(mod.5), sd = sqrt(sum((data$macros_QPB - fitted(mod.5))^2) / (length(data$macros_QPB) - 2)), log = TRUE))
log_likelihood_mod.6 <- sum(dnorm(data$macros_QPB, mean = fitted(mod.6), sd = sqrt(sum((data$macros_QPB - fitted(mod.6))^2) / (length(data$macros_QPB) - 2)), log = TRUE))
log_likelihood_mod.7 <- sum(dnorm(data$macros_QPB, mean = fitted(mod.7), sd = sqrt(sum((data$macros_QPB - fitted(mod.7))^2) / (length(data$macros_QPB) - 2)), log = TRUE))
log_likelihood_mod.8 <- sum(dnorm(data$macros_QPB, mean = fitted(mod.8), sd = sqrt(sum((data$macros_QPB - fitted(mod.8))^2) / (length(data$macros_QPB) - 2)), log = TRUE))


# Calculate AIC and BIC for mod.1
aic_mod.1 <- -2 * log_likelihood_mod.1 + 2 * length(coef(mod.1))
bic_mod.1 <- -2 * log_likelihood_mod.1 + log(length(data$macros_QPB)) * length(coef(mod.1))

# Calculate AIC and BIC for mod.2
aic_mod.2 <- -2 * log_likelihood_mod.2 + 2 * length(coef(mod.2))
bic_mod.2 <- -2 * log_likelihood_mod.2 + log(length(data$macros_QPB)) * length(coef(mod.2))

# Calculate AIC and BIC for mod.3
aic_mod.3 <- -2 * log_likelihood_mod.3 + 2 * length(coef(mod.3))
bic_mod.3 <- -2 * log_likelihood_mod.3 + log(length(data$macros_QPB)) * length(coef(mod.3))

# Calculate AIC and BIC for mod.4
aic_mod.4 <- -2 * log_likelihood_mod.4 + 2 * length(coef(mod.4))
bic_mod.4 <- -2 * log_likelihood_mod.4 + log(length(data$macros_QPB)) * length(coef(mod.4))

# Calculate AIC and BIC for mod.5
aic_mod.5 <- -2 * log_likelihood_mod.5 + 2 * length(coef(mod.5))
bic_mod.5 <- -2 * log_likelihood_mod.5 + log(length(data$macros_QPB)) * length(coef(mod.5))

# Calculate AIC and BIC for mod.6
aic_mod.6 <- -2 * log_likelihood_mod.6 + 2 * length(coef(mod.6))
bic_mod.6 <- -2 * log_likelihood_mod.6 + log(length(data$macros_QPB)) * length(coef(mod.6))

# Calculate AIC and BIC for mod.7
aic_mod.7 <- -2 * log_likelihood_mod.7 + 2 * length(coef(mod.7))
bic_mod.7 <- -2 * log_likelihood_mod.7 + log(length(data$macros_QPB)) * length(coef(mod.7))

# Calculate AIC and BIC for mod.8
aic_mod.8 <- -2 * log_likelihood_mod.8 + 2 * length(coef(mod.8))
bic_mod.8 <- -2 * log_likelihood_mod.8 + log(length(data$macros_QPB)) * length(coef(mod.8))
