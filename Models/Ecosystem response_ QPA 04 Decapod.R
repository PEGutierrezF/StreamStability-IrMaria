...
k=AICc(mod.4, return.K = T)
n=length(decapoda_QPA)
-2*as.numeric(logLik(mod.4))+2*k*(n/(n-k-1))
...
# Gompertz function
gompertz_simple <- function(x, A, d) {
  y = A * exp(-exp(-x)) + d
}

mod.7 <- nlsLM(
  decapoda_QPA ~ gompertz_simple(event, A, d),
  data = data,
  start = list(A = 1, d = 0),
...
n <- length(data$decapoda_QPA)  # Number of observations
